...
public sealed class PluginProjectTests
{
    ...
    {
        var metadata = typeof(MihoyoPlugin).GetCustomAttribute<OhMyBotPluginAttribute>();
    ...
    {
        AssertPackage("Mihoyo");
    }

    [TestMethod]
    public void OwnsItsEfModelAndMigrationLine()
    {
        using var context = new MihoyoDbContext(new DbContextOptionsBuilder<MihoyoDbContext>()
            .UseNpgsql("Host=localhost;Database=unused").Options);

        AssertTables(context, "CoreUsers", "MihoyoAccounts", "MihoyoGameRoles");
